...
module.exports = class BaseDownloadController extends CompositeController {
  ...
  async added() {
    ...
    for (var i = 0; i < requests.length; i++) {
      ...
      let optLM = objectPath.has(request, 'options.headers.last-modified');
      let fileCached = await fs.pathExists(`./download-cache/${this.namespace}/${this.name}/${url}`);
      if (optLM && !fileCached) {
        objectPath.del(request, 'options.headers.last-modified');
      }
      ...
      try {
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          if (resLM) {
            ...
            objectPath.set(request, 'options.headers.last-modified', resLM);
          }
