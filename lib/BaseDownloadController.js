...
module.exports = class BaseDownloadController extends CompositeController {
  ...
  async added() {
    ...
    let ifModifiedSinceArray = objectPath.get(this.data, ['object', 'status', 'last-modifed'], []);
    ...
    for (var i = 0; i < requests.length; i++) {
      ...
      let imsObj = ifModifiedSinceArray.find((el) => objectPath.get(el, 'hash') == requestHash && objectPath.has(el, 'If-Modified-Since'));
      let fileCached = await fs.pathExists(fileCachePath);
      console.log(imsObj);
      if (imsObj && fileCached) {
        objectPath.set(reqOpt, 'headers.If-Modified-Since', objectPath.get(imsObj, 'If-Modified-Since'));
      }
