...
module.exports = class BaseDownloadController extends CompositeController {
  ...
  async added() {
    ...
    for (var i = 0; i < requests.length; i++) {
      ...
      let filename;
      if (git) {
        const parse = gh(git.repo);
        host = parse.host;
        repo = parse.repo;
        branch = git.branch;
        const pattern = parsePath(git.pattern);
        if (pattern.ext == '') {
          if (git.pattern.charAt(git.pattern.length-1) == '*') {
            path = pattern.dir;
          } else {
            path = pattern.path;
          }
        ...
        if (git.gittype == 'github') {
          let enterprise = '';
          if (host == 'github.ibm.com') {
            enterprise = 'http://github.ibm.com/api/v3';
          }
          if (path.charAt(path.length-1) == '/') {
            path = path.substring(0, path.length-1);
          }
          requrl = `GET ${enterprise}/repos/${repo}/contents/${path}?ref=${branch}`;
        ...
        else if (git.gittype == 'gitlab') {
          repo = repo.replace('/', '%2F');
          path = path.replace('/', '%2F');
          requrl = `GET https://${host}/api/v4/projects/${repo}/repository/tree/?path=${path}&ref=${branch}`;
        ...
        files = await request(requrl, { headers: reqOpt.headers });
      ...
      let j = 0;
      let numFiles = files ? files.data.length : 1;
      while (j < numFiles) {
        if (files) {
          if (parsePath(files.data[j].name).ext == fileExt || files.data[j].name == filename || fileExt == '') {
            if (git.gittype == 'github') {
              ...
              let reqglpath = path;
              if (path != '' && path.substring(path.length-3) != '%2F') {
                reqglpath = path + '%2F';
              }
              url = `https://${host}/api/v4/projects/${repo}/repository/files/${reqglpath}${files.data[j].name}/raw?ref=${branch}`;
              reqOpt = { ...reqOpt, url: url };
            } 
          }
