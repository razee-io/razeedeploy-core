...
module.exports = class BaseDownloadController extends CompositeController {
  ...
  async added() {
    ...
    for (var i = 0; i < requests.length; i++) {
      ...
      try {
        request = await this._fetchHeaderSecrets(clone(requests));
      } catch (e) {
  ...
  async _fetchHeaderSecrets(requests) {
    let newRequests = await Promise.all(requests.map(async r => {
      ...
      if (headers) {
        for (let [hKey, hObject] of Object.entries(headers)) {
          ...
          if (secretRef) {
            ...
            if (!secretValue) {
              throw Error(`Unable to fetch header secret data. { name: ${secretName}, namespace: ${secretNamespace }, key: ${secretKey} }`);
            }
            objectPath.set(headers, [hKey], secretValue);
          }
        }
      }
      return r;
