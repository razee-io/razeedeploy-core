...
module.exports = class ReferencedResourceManager {
  ...
  async added() {
    ...
    {
      const fetchEnvs = new FetchEnvs(this);
      const envSources = await fetchEnvs.getSourceSimpleLinks('spec');
      ...
      for (const sourceKind of Object.keys(envSources)){
        ...
        if (! sourceWatchCache.has(sourceKind)){//sourceKind not in watch cache
          const sourceApiVersion = envSources[sourceKind][0].split(':')[0];
          const resourceKrm = await this._kc.getKubeResourceMeta(sourceApiVersion, sourceKind, 'watch');
          const sourceWatchEntry = { watch: this.createWatch(resourceKrm), sources: envSources[sourceKind] };
          sourceWatchCache.set(sourceKind, sourceWatchEntry);
        }
        else {//sourceKind is in watch cache
          const sourceWatchEntry = sourceWatchCache.get(sourceKind);
          //console.log(`CURRENTSOURCELIST: ${JSON.stringify(currentSourceList, null, ' ')}`);
        ...
        for (const sourceSimpleLink of envSources[sourceKind])
        {
          ...
          {
            const sourceInfoEntry = sourceInfoCache.get(sourceSimpleLink);
            ...
            const sourceInfoEntry = { parents: [this._selfLink], lastUpdateTimestamp: lastUpdateTimestamp, resourceVersion: -1 };
            sourceInfoCache.set(sourceSimpleLink, sourceInfoEntry);
          }
        }
      }
      ...
      if (sourceInfoCache.has(this._simpleLink)){
        const sourceData = sourceInfoCache.get(this._simpleLink);
        if (sourceData.parents){
          if( this._resourceVersion > sourceData.resourceVersion)
          {
            for (const parentSelfLink of sourceData.parents){
    ...
    if ( this._managedResourceType === 'parent' )
    {
      ...
      for (const sourceKind of Object.keys(envSources)){
        ...
        for (const sourceSimpleLink of envSources[sourceKind])
        {
          ...
          {
            const sourceData = sourceInfoCache.get(sourceSimpleLink);
            const sourceParentList = sourceData.parents;
  ...
  createWatch(resourceKrm, querySelector = {}, globalWatch = false)
  {
    ...
    const resourceWatch = WatchManager.ensureWatch(options, (data) => this.sourceEventHandler(data, resourceKrm), globalWatch);
  ...
  getCurrentWatches()
  {
    const currentWatchData = WatchManager.getAllWatches();
    const currentWatchSelfLinks = Object.keys(currentWatchData);
    return currentWatchSelfLinks;
  }

  async updateParentSourceTimestamp(parentResourceSelfLink) {
    if (parentResourceSelfLink){
      const sourceUpdateTimestamp = `${new Date().getTime()}`;
      const parentResourceNamespace = parentResourceSelfLink.split('namespaces/')[1].split('/')[0];
      const parentResourceName = parentResourceSelfLink.split('/').pop();
      const parentPatch = {
        metadata: {
          labels: {
      ...
      const res = await this._parentKrm.mergePatch(parentResourceName, parentResourceNamespace, parentPatch, opt);
  ...
  async _modified() {
    // If the modified object does not have a razee data-hash, process the change
    if ( !objectPath.has(this._data, ['object', 'metadata', 'annotations', 'deploy.razee.io/data-hash']))
    {
      await this.modified();
    }
    // If the modified object does have a raze data-hash, only process the change if the data-hash changed (avoids processing the change for `status` changes and the like)
    else{
      // if data, deemed important, has changed (identified via the data hash), modified() should run
      const dh = objectPath.get(this._data, ['object', 'metadata', 'annotations', 'deploy.razee.io/data-hash']);
      const cdh = this._computeDataHash(objectPath.get(this._data, 'object'));
      if (dh != cdh) {
