...
module.exports = class BaseTemplateController extends CompositeController {
  ...
  async _lookupDataReference(valueFrom) {
    ...
    if (valueFrom['secretKeyRef']) {
      ...
      let namespace = objectPath.get(valueFrom, 'secretKeyRef.namespace');
      key = objectPath.get(valueFrom, 'secretKeyRef.key');
      uri = `/api/v1/namespaces/${namespace || this.namespace}/secrets/${secretName}`;
      let res;
      try {
      ...
      let namespace = objectPath.get(valueFrom, 'configMapKeyRef.namespace');
      key = objectPath.get(valueFrom, 'configMapKeyRef.key');
      uri = `/api/v1/namespaces/${namespace || this.namespace}/configmaps/${name}`;
      let res;
      try {
        res = await this.kubeResourceMeta.request({ uri: uri, json: true });
      } catch (e) {
      ...
      let namespace = objectPath.get(valueFrom, 'genericKeyRef.namespace');
      let resource = {};
      if (krm) {
        try {
          resource = await krm.get(name, namespace || this.namespace);
        } catch (e) {
          this.log.warn(e);
        }
      }
  ...
  async _getMaps(envFrom) {
    ...
    await Promise.all(envFrom.map(async (element) => {
      ...
      if (objectPath.has(element, 'configMapRef')) {
        ...
        let namespace = objectPath.get(element, 'configMapRef.namespace');
        let res;
        try {
          res = await this.kubeResourceMeta.request({ uri: `/api/v1/namespaces/${namespace || this.namespace}/configmaps/${name}`, json: true });
        } catch (e) {
          this.log.warn(e);
        }
        ...
        let namespace = objectPath.get(element, 'secretMapRef.namespace');
        let res;
        try {
          res = await this.kubeResourceMeta.request({ uri: `/api/v1/namespaces/${namespace || this.namespace}/secrets/${name}`, json: true });
        } catch (e) {
          this.log.warn(e);
        }
        if (res && res.data) {
        ...
        let namespace = objectPath.get(element, 'genericMapRef.namespace');
        let resource = {};
        if (krm) {
          try {
            resource = await krm.get(name, namespace || this.namespace);
