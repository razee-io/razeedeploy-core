...
module.exports = class BaseTemplateController extends CompositeController {
  ...
  getResourceRefs(){
    const result=[];
    const envFrom = objectPath.get(this.data, 'object.spec.envFrom', []);
    const envFromResources = envFrom.map( (element) => {
      const currentReference = {};
      if (objectPath.has(element, 'configMapRef')) {
        currentReference.apiVersion='v1';
        currentReference.kind='ConfigMap';
        currentReference.name = objectPath.get(element, 'configMapRef.name');
        currentReference.namespace = objectPath.get(element, 'configMapRef.namespace', this.namespace);
      } else if (objectPath.has(element, 'secretMapRef')) {
        currentReference.apiVersion='v1';
        currentReference.kind='Secret';
        currentReference.name = objectPath.get(element, 'secretMapRef.name');
        currentReference.namespace = objectPath.get(element, 'secretMapRef.namespace', this.namespace);
      } else if (objectPath.has(element, 'genericMapRef')) {
        currentReference.apiVersion = objectPath.get(element, 'genericMapRef.apiVersion');
        currentReference.kind = objectPath.get(element, 'genericMapRef.kind');
        currentReference.name = objectPath.get(element, 'genericMapRef.name');
        currentReference.namespace = objectPath.get(element, 'genericMapRef.namespace', this.namespace);
      }
      return currentReference;
    });

    let env = objectPath.get(this.data, 'object.spec.env', []);
    env=env.filter(ref => objectPath.has(ref,'valueFrom'));
    const envResources = env.map( (element) => {
      const currentReference = {};
      if (objectPath.has(element,'valueFrom.secretKeyRef')) {
        currentReference.name = objectPath.get(element,'valueFrom.secretKeyRef.name');
        currentReference.namespace = objectPath.get(element, 'valueFrom.secretKeyRef.namespace', this.namespace);
        currentReference.apiVersion='v1';
        currentReference.kind='Secret';
      } else if (objectPath.has(element,'valueFrom.configMapKeyRef')) {
        currentReference.name = objectPath.get(element, 'valueFrom.configMapKeyRef.name');
        currentReference.namespace = objectPath.get(element, 'valueFrom.configMapKeyRef.namespace', this.namespace);
        currentReference.apiVersion='v1';
        currentReference.kind='ConfigMap';
      } else if (objectPath.has(element,'valueFrom.genericKeyRef')) {
        currentReference.name = objectPath.get(element, 'genericMapRef.name');
        currentReference.namespace = objectPath.get(element, 'genericMapRef.namespace', this.namespace);
        currentReference.kind = objectPath.get(element, 'genericMapRef.kind');
        currentReference.name = objectPath.get(element, 'genericMapRef.name');
  ...
  async added() {
    this.log.info(this.getResourceRefs());
