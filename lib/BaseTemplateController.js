  ...
  async added() {
    ...
    for (var i = 0; i < templates.length; i++) {
      ...
      if (!rsp.statusCode || rsp.statusCode < 200 || rsp.statusCode >= 300) {
        this.log.error(rsp);
        return Promise.reject(`${objectPath.get(templates, [i, 'apiVersion'])}/${objectPath.get(templates, [i, 'kind'])} ${objectPath.get(templates, [i, 'metadata', 'name'])} status ${rsp.statusCode} ${objectPath.get(rsp, 'body.reason', '')}.. see logs for details`);
      }
    }
