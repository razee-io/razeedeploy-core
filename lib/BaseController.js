...
module.exports = class BaseController {
  ...
  get reconcileDefault() {
    return objectPath.get(this._data, ['object', 'metadata', 'labels', 'kapitan.razee.io/Reconcile'], 'true');
  }
  get continueExecution() {
    return this._continueExecution;
  }
  get selfLink() {
    return objectPath.get(this._data, 'object.metadata.selfLink');
  }
  ...
  async execute() {
    try {
      ...
      this._logger.info(`${this._data.type} event recieved ${this.selfLink} ${objectPath.get(this._data, 'object.metadata.resourceVersion')}`);
  ...
  async _added() {
    this._logger.debug(`Finalizer ${this.selfLink}`);
    let deletionTimestamp = await this.finalizer();
    this._logger.debug(`Finalizer ${this.selfLink}: deletionTimestamp ${deletionTimestamp}, continueExecution ${this.continueExecution}`);
  ...
  async finalizer() {
    ...
    if (hasDeletionTimestamp) {
      ...
      if (!this._finCleanRunning && finalizerIndex > -1) {
        ...
        try {
          this._logger.debug(`FinalizerCleanup ${this.selfLink}`);
  ...
  async patchSelf(patchObject, options = {}) {
    ...
    // TODO: get self live resourceVersion.
    // if (originalRv != live) {this._continueExecution = false; and return this._continueExecution;}
  ...
  async apply(krm, file, options = {}) {
    ...
    if (liveResource) {
      let debug = objectPath.get(liveResource, ['metadata', 'labels', 'kapitan.razee.io/debug'], 'false');
