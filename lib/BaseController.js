...
module.exports = class BaseController {
  ...
  async execute() {
    try {
      ...
      if (clusterLocked) {
        ...
        this.updateStatus({ path: ['info', '-'], status: { 'cluster-locked': clusterLocked } });
  ...
  async _added() {
    // must have data-hash to start with to avoid conflict with modified events
    if (!objectPath.has(this._data, ['object', 'metadata', 'annotations', 'deploy.razee.io/data-hash'])) {
      this._logger.debug(`Annotation deploy.razee.io/data-hash not found.. adding.. ${objectPath.get(this._data, 'object.metadata.resourceVersion')}`);
      let res = await this.patchSelf({ metadata: { annotations: { 'deploy.razee.io/data-hash': this.computeDataHash(objectPath.get(this._data, 'object')) } } });
    ...
    this._logger.debug(`Finalizer ${selfLink}: deletionTimestamp ${hasDeletionTimestamp}`);
    if (hasDeletionTimestamp) {
      return await this._patchStatus(); // clear out msgs we dont want to show up after our controller is done cleaning up for this resource (controller logs section in status)
    }

    this._logger.debug(`added() ${selfLink}`);
    await this.added();
    this._logger.debug(`added() completed ${selfLink}`);
    return await this._patchStatus(); // clear out logs from "controller logs" section in status that weren't created this cycle
  }
  async added() {
  ...
  async _modified() {
    ...
    let cdh = this.computeDataHash(objectPath.get(this._data, 'object'));
    if (dh != cdh) {
      ...
      await this.modified();
    } else if (objectPath.has(this._data, 'object.metadata.deletionTimestamp')) {
      this._logger.debug(`Found deletionTimestamp.. running finalizer.. ${objectPath.get(this._data, 'object.metadata.resourceVersion')}`);
      this._logger.debug(`Finalizer ${objectPath.get(this._data, 'object.metadata.selfLink')}`);
      let hasDeletionTimestamp = await this.finalizer();
      this._logger.debug(`Finalizer ${objectPath.get(this._data, 'object.metadata.selfLink')}: deletionTimestamp ${hasDeletionTimestamp}`);
      return await this._patchStatus(); // clear out msgs we dont want to show up after our controller is done cleaning up for this resource (controller logs section in status)
  ...
  async finalizer() {
    ...
    if (hasDeletionTimestamp) {
      ...
      if (finalizerIndex > -1) {
        ...
        await this.patchSelf({ metadata: { resourceVersion: objectPath.get(this._data, 'object.metadata.resourceVersion'), finalizers: finalizers } });
        // if patch to remove finalizer fails (will fail if resourceVersion has changed), this will reject out and try again next cycle.
      }
  ...
  async _patchStatus() {
    // TODO:
    // track which msgs we created this cycle (msg hash) and delete all others

    objectPath.set(this._status, 'fatal', objectPath.get(this._status, 'fatal', null));
    objectPath.set(this._status, 'error', objectPath.get(this._status, 'error', null));
    objectPath.set(this._status, 'warn', objectPath.get(this._status, 'warn', null));
    objectPath.set(this._status, 'info', objectPath.get(this._status, 'info', null));
