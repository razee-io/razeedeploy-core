...
module.exports = class BaseController {
  ...
  async _added() {
    ...
    if (!objectPath.has(this._data, ['object', 'metadata', 'annotations', 'deploy.razee.io/data-hash'])) {
      await this.patchSelf({ metadata: { annotations: { 'deploy.razee.io/data-hash': this.computeDataHash() } } });
    }
  ...
  computeDataHash() {
    ...
    let resource = objectPath.get(this._data, 'object');
    let importantData = {
  ...
  async _patchStatus() {
    // TODO:
    // track which msgs we created this cycle (UUID?) and delete all others
    // possiblly also associate the dataHash that created the error
