...
module.exports = class BaseController {
  ...
  async _added() {
    ...
    if (hasDeletionTimestamp) {
      if (objectPath.get(this._data, 'object.metadata.finalizers', []).length > 0) {
        try {
          await this._patchStatus();
          return await this._patchRazeeLogs();
        } catch (e) {
  ...
  async _patchRazeeLogs() { // clear out logs in status that weren't created this cycle
    let patchObj = {};
  ...
  // this function can be removed when we move away from kapitan code
