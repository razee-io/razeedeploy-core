...
module.exports = class BaseController {
  ...
  async _added() {
    ...
    this._logger.debug(`Added Finalizer ${selfLink}`);
  ...
  async _modified() {
    if (objectPath.has(this._data, 'object.metadata.deletionTimestamp')) {
      ...
      this._logger.debug(`Modified Finalizer ${objectPath.get(this._data, 'object.metadata.selfLink')}`);
      let hasDeletionTimestamp = await this.finalizer();
  ...
  async updateRazeeLogs(logLevel, log) {
    let patchObj = {};
  ...
  async _patchRazeeLogs() {
    let patchObj = {};
    let logLevels = objectPath.get(this._data, 'object.status.razee-logs', {});
    Object.values(logLevels).map(logLevel => {
      let logHashes = Object.values(logLevel);
      logHashes.map(logHash => {
        if (!this._razeeLogHashes.includes(logHash)) {
