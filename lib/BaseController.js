...
module.exports = class BaseController {
  ...
  reconcileFields(config, lastApplied, parentPath = []) {
    ...
    Object.keys(lastApplied).forEach(key => {
      ...
      path.push(key);
      // if new config does not have lastApplied path, make sure to null it out.
      if (!objectPath.has(config, path)) {
        objectPath.set(config, path, null);
        // elseif (lastApplied[key] is not null/undefined, it is an object, and it is not an array) AND (the same path in the original config is also not null/undefined, is of also of type object, and is not an array)
      } else if ((lastApplied[key] && typeof lastApplied[key] == 'object' && !Array.isArray(lastApplied[key])) && (objectPath.get(config, path) && typeof objectPath.get(config, path) == 'object' && !Array.isArray(objectPath.get(config, path)))) {
        this.reconcileFields(config, lastApplied[key], path);
      } // else it exists in lastApplied and new config, no need to alter it
    });
  }
