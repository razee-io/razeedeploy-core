...
module.exports = class BaseController {
  ...
  async execute() {
    try {
      ...
      if (clusterLocked) {
        this._logger.info(`Cluster Locked ${clusterLocked}.. skipping ${this._data.type} event ${objectPath.get(this._data, 'object.metadata.selfLink')} ${objectPath.get(this._data, 'object.metadata.resourceVersion')}`);
  ...
  async _cluster_locked() {
    ...
    if (exists) {
      ...
      lockCluster = lockCluster.trim();
    }
    return lockCluster == 'true' ? true : false;
  }
  // ===========================================
