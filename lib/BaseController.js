...
module.exports = class BaseController {
  ...
  async execute() {
    try {
      ...
      let cluster_locked = await this._cluster_locked();
      this.updateStatus({ path: ['info', '-'], status: { 'cluster-locked': cluster_locked } });
      if (cluster_locked) {
  ...
  async _cluster_locked() {
    ...
    if (exists) {
      let lockCluster = await fs.readFile(lockClusterPath);
