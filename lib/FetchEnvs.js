...
module.exports = class FetchEnvs {
  ...
  #processEnvSourceSimpleLinks(envs) {
    return Promise.all(envs.map(async (env) => {
      if (env.value) return env;
      let parentNamespace = this.namespace;
  ...
  async getSourceSimpleLinks(path = 'spec') {
    ...
    result = await (await this.#processEnvSourceSimpleLinks(envFrom)).reduce(reduceEnvSourceSimpleLinks, result);
...
function reduceEnvSourceSimpleLinks(output, conf) {
  ...
  if (valueFrom !== undefined) {
    let sourceValueFromInfo = Object.values(valueFrom)[0];
    let sourceKind;
    if( objectPath.get(sourceValueFromInfo, 'kind') ) //sourceValueFromInfo has a kind property, source is genericMapRef
    {
      sourceKind = objectPath.get(sourceValueFromInfo, 'kind');
    }
    else{ //else: sourceValueFromInfo doesn't have kind property, source is configMapRef or secretMapRef, use KIND_MAP to find which
      sourceKind = KIND_MAP.get(Object.keys(valueFrom)[0]);
    }
    let sourceNamespace = sourceValueFromInfo.namespace || parentNamespace;
    let sourceApiVersion = sourceValueFromInfo.apiVersion || 'v1';
    let sourceSimpleLink = `${sourceApiVersion}:${sourceKind}/${sourceNamespace}/${sourceValueFromInfo.name}`;
    let sourceData = {[sourceKind]: [sourceSimpleLink]};
