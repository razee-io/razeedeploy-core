...
module.exports = class FetchEnvs {
  constructor(data, krm, kc, updateRazeeLogs) {
    if (!(data && krm && kc)) {
    ...
    this.updateRazeeLogs = updateRazeeLogs || (() => { log.debug('\'updateRazeeLogs()\' not passed to fetchEnvs. will not update razeeLogs on failure to fetch envs'); });
  }
