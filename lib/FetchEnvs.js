...
module.exports = class FetchEnvs {
  constructor(controllerObject) {
    ...
    this.kubeResourceMeta = controllerObject.krm;
    this.kubeClass = controllerObject.kc;

    this.updateRazeeLogs = controllerObject.updateRazeeLogs || (() => { log.debug('\'updateRazeeLogs()\' not passed to fetchEnvs. will not update razeeLogs on failure to fetch envs'); });
  }
