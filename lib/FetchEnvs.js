...
const singleResourcePLimits = {};
const matchLabelsLimit = pLimit( 10 );  // No more than this many simultaneous matchLabels queries allowed.
...
module.exports = class FetchEnvs {
  ...
  */
  async #genericMapRef(conf, valueFrom = 'genericMapRef', decode = false) {
    ...
    // Ensure same resource is not being retrieved multiple times simultaneously, for optimal cache utilization.
    let limit;
    if( !Object.prototype.hasOwnProperty.call( singleResourcePLimits, cacheKey ) ) singleResourcePLimits[cacheKey] = pLimit(1);
    limit = singleResourcePLimits[cacheKey];

    let { resource, kubeError } = await limit(async () => {
      let resource, kubeError;
      ...
      return( { resource, kubeError } );
    });
    if( !kubeError ) kubeError = ERR_NODATA;
  ...
  */
  async #genericKeyRef(conf, valueFrom = 'genericKeyRef', decode = false) {
    ...
    // Ensure same resource is not being retrieved multiple times simultaneously, for optimal cache utilization.
    // MatchLabels queries return multiple resources, and are limited separately via `matchLabelsLimit`.
    let limit;
    if (typeof matchLabelsQS === OBJECT) {
      limit = matchLabelsLimit;
    }
    else {
      if( !Object.prototype.hasOwnProperty.call( singleResourcePLimits, cacheKey ) ) singleResourcePLimits[cacheKey] = pLimit(1);
      limit = singleResourcePLimits[cacheKey];
    ...
    let { response, kubeError } = await limit(async () => {
      let response, kubeError;
